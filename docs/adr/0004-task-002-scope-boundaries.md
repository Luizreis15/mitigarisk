# ADR 0004 — TASK-002 scope boundaries and deferred decisions
...
- **The exact role → capability grants.** Seeded as a reasonable default
  (ADR 0002) because the task explicitly requires roles/capabilities to
...
- **Real Postgres RLS test execution.** Rather than only asserting
  ...
  `app` schema PostgREST-exposure defect described in ADR 0002.
