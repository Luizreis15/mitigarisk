# ADR 0004 — Immutable evidence via triggers, not application discipline alone
...
- `policy_versions` (once `published`/`archived`), their `policy_factors`/
  `policy_thresholds`, `evaluations` (including immutable input identity
  while pending and all fields once `completed`/`failed`), their
...
- An evaluation's `decision_band` is a recommendation column with no
  ...
  acted on."
- Webhook delivery and notification-request identity, payload, recipient,
  correlation, and idempotency evidence cannot be rewritten by a delivery
  worker. Only delivery-state fields may change through a privileged
  server-side worker; authenticated users have no update or delete policy.
...
- These triggers, tenant-linkage constraints, and the
  `record_audit_event` attribution guarantee are exercised against a real
  (disposable, local, credential-free) Postgres instance in
  `supabase/tests/010-rls-tenant-isolation.sql` and
  `supabase/tests/020-integration-hardening.sql` — see
