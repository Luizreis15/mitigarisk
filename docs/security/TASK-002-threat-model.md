...
| T12 | A tenant manager deletes an API client, webhook endpoint, notification template, or delivery history despite the retention model. | Broad `FOR ALL` policies were replaced with operation-specific INSERT/UPDATE policies. Delivery histories have no authenticated UPDATE/DELETE policy, and immutable identity/payload triggers constrain privileged worker updates. |
| T13 | A record claims one `tenant_id` while referencing a policy or evaluation owned by another tenant. | Evaluations and cases use composite foreign keys that include `tenant_id`; negative cross-tenant reference tests run in `020-integration-hardening.sql`. |
| T14 | A user keeps acting after their tenant is suspended. | `app.current_tenant_ids()` and `app.has_capability()` require both an active membership and an active tenant. |
| T15 | An authenticated tenant member creates a tenant-less platform audit event. | `public.record_audit_event()` accepts `tenant_id = null` only for a user present in `platform_admins`. |
...
- Role → capability seed values (`docs/adr/0003-capability-based-authorization.md`)
  are an implementer default pending product confirmation.
- Hash columns currently remain in the public-schema table shape. Before
  real API-key or webhook-secret issuance, move hashes behind a private
  server-only repository or add explicit column-level exposure tests.
