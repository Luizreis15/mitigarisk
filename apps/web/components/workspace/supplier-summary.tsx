...
function formatExposure(supplier: Supplier): string {
  const majorUnits = (supplier.annualExposureMinor / 100).toLocaleString('en-US', {
...
export function SupplierSummary({ supplier }: { supplier: Supplier }) {
  ...
  return (
    <section aria-labelledby="supplier-summary" className="space-y-2">
      <h2 id="supplier-summary" className="text-sm font-medium">
        {t.summaryTitle}
      </h2>
      <dl className="grid grid-cols-1 gap-x-4 gap-y-2 text-sm sm:grid-cols-2">
        <div>
          <dt className="text-muted-foreground">{t.reference}</dt>
          <dd className="font-mono text-xs">{supplier.reference}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t.status}</dt>
          <dd>{t.statusValues[supplier.status]}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t.fields.registrationCountryCode}</dt>
          <dd>{supplier.registrationCountryCode}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t.fields.registrationIdentifier}</dt>
        ...
        <div className="sm:col-span-2">
          <dt className="text-muted-foreground">{t.relationshipPurposeLabel}</dt>
