...
4. Every numbered verification suite after `000`, currently:
   - `010-rls-tenant-isolation.sql` — authorized access,
     unauthenticated denial, cross-tenant denial, least privilege, and
     published-policy immutability.
   - `020-integration-hardening.sql` — English-first defaults,
     suspended-tenant denial, cross-tenant reference integrity, provenance,
     immutable evidence, and retained delivery history.
